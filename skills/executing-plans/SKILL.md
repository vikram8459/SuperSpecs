...
description: Use when you have an OpenSpec change folder + plan and want to execute tasks inline (in this session) with human checkpoints. Pairs with openspec-apply for spec-driven discipline.
---
...
**Note:** SuperSpecs works much better with subagent support. If your platform supports `Task`-style subagent dispatch, prefer `superspecs:subagent-driven-development` over this skill.

**Required companion skill:** `superspecs:openspec-apply` — it defines the per-task discipline (read spec → implement → verify scenarios → commit). This skill defines the **dispatch model** (inline, batched, with human checkpoints).
...
For each task, follow `superspecs:openspec-apply`:
...
Wait for the user's go-ahead before continuing. This is the main difference from subagent-driven-development: the human stays in the loop.

### Step 4: Finish

After all tasks complete and verified:
- Announce: "I'm using the finishing-a-development-branch skill."
- **REQUIRED SUB-SKILL:** `superspecs:finishing-a-development-branch`
- After merge: **REQUIRED SUB-SKILL:** `superspecs:openspec-archive`
...
**Ask for clarification rather than guessing.** If the spec needs to change, follow the "When the Spec Is Wrong" guidance in `superspecs:openspec-apply`.

## When to Revisit Earlier Steps

**Return to Step 1 when:**
- The user updates the plan or spec deltas based on your feedback
- The fundamental approach needs rethinking — escalate to brainstorming/openspec-propose
...
- Follow `openspec-apply` exactly per task
...
- **superspecs:openspec-propose** — produces the change folder
- **superspecs:writing-plans** — produces the plan this skill executes
- **superspecs:openspec-apply** — drives each task
...
- **superspecs:openspec-archive** — fold deltas into active spec set after merge
